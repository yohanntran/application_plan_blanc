...
# Fichier annuaire
annuaire_path <- file.path("..", "DB", "annuaire.xlsx")
if (file.exists(annuaire_path)) {
  annuaire <- normalizePath(annuaire_path, mustWork = TRUE)
} else {
  annuaire <- NULL
...
# Fichier reponsesSMS
reponsesSMS_path <- file.path("..", "DB", "reponsesSMS.xls")
if (file.exists(reponsesSMS_path)) {
  reponsesSMS <- normalizePath(reponsesSMS_path, mustWork = TRUE)
} else {
  reponsesSMS <- NULL
...
traitement_SMS <- function(simulation = FALSE) {
  ...
  return(list(
    ...
    pas_present = pas_present,
    last_reponseSMS = last_reponseSMS,
    recent_annuaire = recent_annuaire,
    nombre_personnel = nombre_personnel
  ))
}

traitement_SMS_safe <- function(simulation = FALSE) {
  result <- tryCatch(
    {
      # On appelle ta fonction normale
      traitement_SMS(simulation = simulation)
    },
    error = function(e) {
      message("⚠️ Erreur lors du traitement, création de tableaux vides...")
      
      # Création de data.frames vides pour toutes les variables
      empty_df <- data.frame()
      list(
        Base_PB = empty_df,
        ...
        last_reponseSMS = empty_df,
        recent_annuaire = empty_df,
        nombre_personnel = 0
      )
    }
  )
...
traitement_donnees_complet <- function(simulation = F){
  ...
  colnames(Base_PB_ide)=c("Identité","Numéro de téléphone", "Hôpital", "Fonction", "Message d'origine","Disponibilité","Contact")
  colnames(Base_PB_medecin)=c("Identité","Numéro de téléphone", "Hôpital", "Fonction", "Message d'origine","Disponibilité","Contact")
  colnames(Base_PB_arm)=c("Identité","Numéro de téléphone", "Hôpital", "Fonction", "Message d'origine","Disponibilité","Contact")
  colnames(Base_PB_cadre)=c("Identité","Numéro de téléphone", "Hôpital", "Fonction", "Message d'origine","Disponibilité","Contact")
  colnames(Base_PB_chir)=c("Identité","Numéro de téléphone", "Hôpital", "Fonction", "Message d'origine","Disponibilité","Contact")
  colnames(Base_PB_pilot)=c("Identité","Numéro de téléphone", "Hôpital", "Fonction", "Message d'origine","Disponibilité","Contact")
  colnames(Base_PB_secu)=c("Identité","Numéro de téléphone", "Hôpital", "Fonction", "Message d'origine","Disponibilité","Contact")
  colnames(Base_PB_ibode)=c("Identité","Numéro de téléphone", "Hôpital", "Fonction", "Message d'origine","Disponibilité","Contact")
  colnames(Base_PB_autre)=c("Identité","Numéro de téléphone", "Hôpital", "Fonction", "Message d'origine","Disponibilité","Contact")
